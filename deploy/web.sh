...
docker run -d --name="web" --restart="on-failure" --publish="80:80" --publish="443:443" --link="back:back" --volume="/etc/nginx/nginx.conf:/etc/nginx/nginx.conf" --env="CONFIG=$1" quay.io/discipuluz/discipuluz-web:$1